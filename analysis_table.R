...
mean_and_sd_for_prices <- function(my_datas, first_date = Sys.Date() -365, last_date = Sys.Date() , sample_size){
  datas <- lapply(names(my_datas), function(x){
    ...
    if(length(which(time_filter))!= 0){
      my_datas <- central_limit_theorem(na.exclude(my_datas[[x]][["price"]]$adjusted[time_filter]), sample_size = sample_size )
      return(my_datas)
    }else{
...
central_limit_theorem <- function(vector_data, sample_size){
  means_samples <- c()
  price_vector <- na.exclude(vector_data)
  while(length(price_vector)> 0){
    if(length(price_vector)< sample_size){
      sample_size = length(price_vector)
    }
  
    sample_location <- sample(1:length(price_vector), sample_size)
  ...
  return(list(my_mean = mean(means_samples), means_samples = means_samples, sd = sd(means_samples), var(means_samples)))
}
#test4 <- central_limit_theorem(vector_data = test[[4]]$price$adjusted, sample_size = 5)
#test4$my_mean
#plot(table(round((test4$means_samples - test4$my_mean)/test4$sd, 1)))

#sd(means_samples - my_mean)
#sd(na.exclude(test[[4]]$price$adjusted))
#test3 <- analysis_table(price_and_dividends_data = price_dividend_data[1:150], first_date = Sys.Date() -1000, last_date = Sys.Date(), sample_size = 5)
#test5 <- mean_and_sd_for_prices(test, sample_size = 5)
