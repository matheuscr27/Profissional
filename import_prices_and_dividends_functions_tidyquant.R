...
#  prices <- list_data_price[[x]][['df.tickers']]
 ...
 # print("-------------")
  ...
  last_365_day <- as.Date((last_date - 365):last_date)
...
import_datas <- function(tickers, 
  ...
  list_table_dividends <- lapply(names(list_data_dividend), function(x){
      ...
      if(nrow(list_data_dividend[[x]]) == 0){
        ...
        print(paste("nrow =", nrow(list_data_dividend[[x]])))
...
#test2 <- filter_price_dividend_by_date(import_datas = test, first_date = Sys.Date() - 365, last_date = Sys.Date(), tickers = paste0(test_tickers$ticket[1:10],".SA"))
